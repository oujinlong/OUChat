...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    [UserModel UserModelWithEaseMobUserName:[EMClient sharedClient].currentUsername isUpdate:NO hanlde:^(UserModel *userModel) {
        //将头像和昵称作为拓展消息传递
        NSDictionary* ext = @{MESSAGE_PORTRAIT_EXT_KEY:userModel.portrait,USER_NICKNAME_KEY : userModel.nickName};
        message.ext = ext;
...
-(id<IMessageModel>)messageViewController:(EaseMessageViewController *)viewController modelForMessage:(EMMessage *)message{
    ...
    model.avatarURLPath = [message.ext valueForKey:MESSAGE_PORTRAIT_EXT_KEY];//头像网络地址
    model.nickname = [message.ext valueForKey:USER_NICKNAME_KEY];//用户昵称
    return model;
}
